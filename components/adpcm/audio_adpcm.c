...
void ADPCMEncodeBuffer(AdpcmCodecDcpt* pDcpt)
{
	...
	if(pDcpt->sLE)
	{
		while(nSamples/2)
		{
			...
			adPCMHi=ADPCMEncodeSample( *srcBuff, &pDcpt->state);//kafuse
            srcBuff ++;
			adPCMLo=ADPCMEncodeSample( *srcBuff, &pDcpt->state);//kafuse
            srcBuff ++;
